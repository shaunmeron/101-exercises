...
let newVeggies = vegetables.sort();
let newFruits = fruits.sort().reverse();
let fruitsAndVeggies = newFruits.concat(newVeggies);
...
// Write a function definition named isVowel that takes in valu2eminee and returns true if the value is a, e, i, o, u in upper or lower case.
...
const isVowel = (str) => {
    if (str.length > 1) {
        ...
        for (const vowel of vowels) {
            if (vowel.includes(str)) return true;
        }
        return false;
    }

    // vowels.forEach((letter) => str === letter) ? true : false;
};
...
const hasVowels = (str) => {
    for (const string of str) {
        for (const vowel of vowels) {
            if (vowel.includes(string)) return true;
        }
    }
    return false;
};
...
function startsWithVowel(str) {
    for (const vowel of vowels) {
        if (str.startsWith(vowel)) return true;
    }
    return false;
}
...
function endsWithVowel(str) {
    for (const vowel of vowels) {
        if (str.endsWith(vowel)) return true;
    }
    return false;
}
...
function startsAndEndsWithVowel(str) {
    if ((startsWithVowel(str) && endsWithVowel(str)) === true) {
        return true;
    }
    return false;
}
...
const productOfAll = (arr) => arr.reduce((a, b) => a * b);
...
const countEvens = (arr) => onlyEvenNumbers(arr).length;
...
const countOdds = (arr) => onlyOddNumbers(arr).length;
...
const countNegatives = (arr) => onlyNegativeNumbers(arr).length;
...
const countPositives = (arr) => onlyPositiveNumbers(arr).length;
...
const onlyPositiveEvens = (arr) => {
    let positiveEvens = [];
    for (const number of arr) {
        if (isPositiveEven(number)) {
            positiveEvens.push(number);
        }
    }
...
const onlyPositiveOdds = (arr) => {
    let positiveOdds = [];
    for (let number of arr) {
        if (isPositiveOdd(number)) {
            positiveOdds.push(number);
        }
    }
    return positiveOdds;
};
...
const onlyNegativeEvens = (arr) => {
    let negativeEvens = [];
    for (let number of arr) {
        if (isNegativeEven(number)) {
            negativeEvens.push(number);
        }
    }
...
const onlyNegativeOdds = (arr) => {
    let negativeOdds = [];
    for (let number of arr) {
        if (isNegativeOdd(number)) {
            negativeOdds.push(number);
        }
    }
    return negativeOdds;
};
...
const elementsTimesTwo = (arr) => arr.map((x) => x * 2);
...
const addOneToArray = (arr) => arr.map((x) => x + 1);
...
const totalOfBookPrices = (x) => {
    let totalPrice = 0;
    x.forEach((obj) => (totalPrice += obj.price));
    return totalPrice;
};
...
const getAverageBookPrice = (arr) => {
    return totalOfBookPrices(arr) / arr.length;
};
...
const highestPriceBook = (arr) => {
    let highestPrice = arr.reduce((max, book) => (max.price > book.price ? max : book));
    return highestPrice;
};
...
const lowestPriceBook = (arr) => {
    let lowestPrice = arr.reduce((max, book) => (max.price > book.price ? book : max));
    // (max.price > book.price ? book : max));
    return lowestPrice;
};
...
const getTaxRate = (obj) => shoppingCart.tax;
...
let totalQuantity = shoppingCart.items.reduce(function (sum, number) {
    return sum + number.quantity;
}, 0);

const totalNumberOfItems = (obj) => totalQuantity;
/*  SHAUN'S NOTE: Because we are ACCUMULATING values rather than COMPARING values like in the highestPriceBook challenge, we pass in 0 as our second argument as a starting tally. Our "sum" variable in this instance will start from 0, and accumulate every iteration of number.quantity. The sum variable doesn't need the ".quantity" property because it is just a count, it is not pointing to an iterated value in the array. Similar to syntax:
emptyArray += arr[i]
 where emptyArray is an initialized counter variable and arr[i] is each iterated value in a loop. 
EX: 0 + 1 -> 1
    1 + 3 -> 4 
    4 + 3 -> 7
etc. 
...
let sumPrice = shoppingCart.items.reduce(function (sum, current) {
    return sum + current.price;
}, 0);

const getAverageItemPrice = (obj) => sumPrice / obj.items.length;
